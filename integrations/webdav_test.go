// Copyright 2021 Evgeni Golov and thehe goftp Authors. All rights reserved.
...
import (
	...
	webdav_server "golang.org/x/net/webdav"
)

func TestWebdavDriver(t *testing.T) {
	err := os.MkdirAll("./testdata", os.ModePerm)
	assert.NoError(t, err)

	webdav_handler := &webdav_server.Handler{
		Prefix:     "/",
		FileSystem: webdav_server.Dir("./testdata"),
		LockSystem: webdav_server.NewMemLS(),
		Logger:     nil,
	}

	http.HandleFunc("/", webdav_handler.ServeHTTP)
	go func() {
		if err := http.ListenAndServe("localhost:8080", webdav_handler); err != nil {
			// handle err
		}
	}()

	var perm = server.NewSimplePerm("test", "test")
	driver, err := webdav.NewDriver("http://localhost:8080", "admin", "admin", true)
	assert.NoError(t, err)
